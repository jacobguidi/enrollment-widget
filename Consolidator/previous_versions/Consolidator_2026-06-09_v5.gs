...
// CHANGE LOG
// ============================================================
// 2026-06-09 v5 — Add _parseRawDate() to handle PPS date formats: M.D.YY (dots) and MDDYYYY (7-digit).
//                 Plugged into _formatDate and all raw date parsing in _pullFromPPS.
...
// Handles PPS date formats before falling back to new Date():
//   M.D.YY   e.g. 3.1.23  → 03/01/2023
//   MDDYYYY  e.g. 3012024 → 03/01/2024  (7 digits: 1-digit month + 2-digit day + 4-digit year)
//   MMDDYYYY e.g. 03012024→ 03/01/2024  (8 digits)
function _parseRawDate(val) {
  if (!val) return null;
  if (val instanceof Date) return isNaN(val.getTime()) ? null : val;

  const s = String(val).trim();

  // M.D.YY or M.D.YYYY (dot-separated)
  ...
  // MDDYYYY (7 digits) or MMDDYYYY (8 digits) — no separators
  const numMatch = s.match(/^(\d{7,8})$/);
  if (numMatch) {
    const n = s;
    let m, d, y;
    if (n.length === 7) {
      m = n.slice(0, 1); d = n.slice(1, 3); y = n.slice(3);
    } else {
      m = n.slice(0, 2); d = n.slice(2, 4); y = n.slice(4);
    }
    const dt = new Date(+y, +m - 1, +d);
    return isNaN(dt.getTime()) ? null : dt;
  }

  // Fallback: let JS parse it (handles ISO, MM/DD/YYYY, etc.)
...
function _pullFromPPS(workbookId, clientMeta, ts) {
  ...
  for (const row of data) {
    ...
    const termDate = _parseRawDate(termDateRaw);
    const effDate  = _parseRawDate(effDateRaw);
