...
function runMasterConsolidation() {
  ...
  const eligSheet   = masterSS.getSheetByName('Master Eligibility');

  if (!clientSheet) throw new Error('No "Clients" tab in Master Workbook');
  if (!eligSheet)   throw new Error('No "Master Eligibility" tab in Master Workbook');
  ...
  for (let i = 1; i < clientData.length; i++) {
    ...
    const clientMeta = {
      ...
      workbookId:      String(client[cm['Workbook ID']]     || '').trim(),
...
function generateRecuroFile() {
  ...
  const masterFile = DriveApp.getFileById(MASTER_WORKBOOK_ID);
